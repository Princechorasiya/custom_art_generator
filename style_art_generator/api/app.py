...
import threading
import time
from config import PRETRAINED_STYLES  # Import the pre-trained styles
...
@app.route('/api/pretrained_styles', methods=['GET'])
def get_pretrained_styles():
    # Return the list of available pre-trained styles
    styles_list = []
    for style_id, style_info in PRETRAINED_STYLES.items():
        styles_list.append({
            'id': style_id,
            'name': style_info['name'],
            'description': style_info['description'],
            'thumbnail': style_info['thumbnail']
        })
...
def generate_image():
    ...
    style_type = data.get('style_type', 'custom')  # 'custom' or 'pretrained'
    ...
    elif style_type == 'pretrained':
        # Check if this is a valid pre-trained style
        if style_id not in PRETRAINED_STYLES:
            return jsonify({'error': 'Pre-trained style not found'}), 404
        
        lora_path = PRETRAINED_STYLES[style_id]['lora_path']
    else:
        return jsonify({'error': 'Invalid style type'}), 400
